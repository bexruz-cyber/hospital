  ...
  export default function Spec() {
    ...
    useEffect(() => {
      const fetchDepartments = async () => {
        ...
        try {
          const response = await axios.get('http://195.158.9.124:4109/spec', {
            headers: {
    ...
    const handleDelete = async (id: string) => {
      try {
        await axios.delete(`http://195.158.9.124:4109/spec/${id}`, {
          headers: {
            Authorization: `Bearer ${localStorage.getItem('authToken')}`,
          },
        });
        toast({
          title: "Spec delete successfully",
        });
    ...
    const handleAddDepartment = async () => {
      ...
      try {
        const response = await axios.post('http://195.158.9.124:4109/spec', {
    ...
    const handleUpdateDepartment = async () => {
      ...
      try {
        const response = await axios.put(`http://195.158.9.124:4109/spec`, {
        ...
        if (response.data) {
          ...
          toast({
            title: "Spec edited successfully",
          });
        }
      } catch (err: any) {
    ...
    return (
      <div>
        <div className="flex items-center justify-between mb-5">
          <h1 className="text-2xl font-bold">Spec</h1>
