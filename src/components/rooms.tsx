...
export default function DoctorForm() {
  ...
  const handleDeleteDoctor = async (id: string) => {
    ...
    try {
      ...
      setDoctors(updatedDoctors);
      toast({
        title: "Rooms delet successfully",
      });
    } catch (error: any) {
      setError(error.response?.data?.message || error.message);
    }
        ...
        {doctors.map((doctor) => (
          <div key={doctor._id} className="border border-gray-300 rounded p-4">
            <div className="grid grid-cols-2 gap-1 mb-4">
              ...
              <p className='text-xl font-medium'>Gender: {doctor.gender}</p>
              ...
              <p className='text-xl font-medium'>Family Phone: {doctor.familyphone}</p>
              <p className='text-xl font-medium'>Work Time: {doctor.worktime}</p>
            ...
            <form onSubmit={handleAddDoctor} className='max-h-[600px] grid grid-cols-2 gap-5 overflow-y-auto'>
              ...
              <div>
                ...
                >
                  ...
                  {specs.map((spec) => (
                    <option key={spec._id} value={spec._id}>{spec.title}</option>
                  ))}
                </select>
              </div>
              <div>
              ...
              <div className="col-span-2">
                <Button type="submit" className="bg-blue-600 text-white">Add Doctor</Button>
                <Button type="button" className="ml-2 bg-gray-300 text-black" onClick={resetForm}>Cancel</Button>
              </div>
            </form>
          </Dialog.Panel>
        </div>
